...
interface ClienteRecuperacaoProps {
  onClose: () => void;
}

const ClienteRecuperacaoForm: React.FC<ClienteRecuperacaoProps> = ({ onClose }) => {
  ...
  const handleChangeRecuperacao = (e: React.ChangeEvent<HTMLInputElement>) => {
    setFormDataRecuperacao({ ...formDataRecuperacao, [e.target.name]: e.target.value });
  };
...
export default ClienteRecuperacaoForm;
